...
# build — all five carry `package` + `src$file`, which the renderUI dep sink's
# native pipeline resolves and serves at mount time.
...
#' and delivered at mount time through the per-session renderUI dep sink (see
#' `install_widget_dep_sink` in mount.R).
#'
