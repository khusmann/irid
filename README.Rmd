...
Bare `onInput` callbacks are automatically debounced (200 ms) so the server only
sees the settled value. All other events fire immediately. Override with explicit
wrappers:
...
`event_throttle()` and `event_debounce()` support **adaptive coalescing**
(`coalesce = TRUE`, the default): the client also waits for the server to finish
processing before sending the next event, preventing queue buildup when the
server is slow.
