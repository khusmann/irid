...
- **wire** — reserved for the per-channel event-stream config: `wire()`/`wire_*()`, the `wire` op (in `irid-render`), `R/wire.R`. How an event is *wired* to the server, not the serialization.
