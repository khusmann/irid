...
test_that("widget deps are NOT page-attached at render (they ride the sink)", {
  # Deps are delivered at mount time through the per-session renderUI sink, not
  # collected onto the static page by htmltools. So even a statically-placed
  # widget's dep is absent from the processed tag tree; process_tags only
  # carries it on the widget_inits entry for mount to feed the sink.
  # (See test-widget-deps.R for the sink-delivery side.)
