...
Every example also ships with the package, so you can run it locally with `iridExample()`. Pass the name from its link to launch it:

```r
iridExample("old_faithful")
```

Call `iridExample()` with no argument to list the available examples. Some examples need extra packages (e.g. bslib, plotly); the runner checks for these first and tells you what to install. A couple (`plotly`, `codemirror`) load JavaScript from a CDN at runtime, so they need an internet connection even when run locally.
...
### [Old Faithful](../apps/old_faithful/index.html?_shinylive-mode=editor-terminal-viewer){target="_blank"}
...
### [Nested Each](../apps/each_nested/index.html?_shinylive-mode=editor-terminal-viewer){target="_blank"}
...
### [Heterogeneous Each](../apps/each_heterogeneous/index.html?_shinylive-mode=editor-terminal-viewer){target="_blank"}
...
### [Optimistic Updates](../apps/optimistic_updates/index.html?_shinylive-mode=editor-terminal-viewer){target="_blank"}
...
### [Shiny Modules](../apps/shiny_interop/index.html?_shinylive-mode=editor-terminal-viewer){target="_blank"}
